...
listenChange('.isEmployerVerify', function () {
    let companyIsActiveId = $(this).attr('data-id');
    changeIsVerifyRender(companyIsActiveId);
});
...
// change verify 
function changeIsVerifyRender (companyIsActiveId) {
    $.ajax({
        url: route('change.company.is_verify', companyIsActiveId),
        method: 'post',
        cache: false,
        success: function (result) {
            if (result.success) {
