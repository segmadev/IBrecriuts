...
class JobApplicationTable extends LivewireTableComponent
{
    ...
    public function columns(): array
    {
        if(Auth::user()->hasRole("Admin")) {
            ...
            $holder[] =Column::make(__('messages.job_application.candidate_name'), 'candidate.user.first_name')
            ->sortable(function (Builder $query, $direction) {
                return $query->orderBy(User::select('first_name')->whereColumn('candidates.user_id', 'users.id'), $direction);
            })
            ->searchable()
            ->view('employer.job_applications.table_components.admin_candidate_name');
        }else{
            $holder[] = Column::make(__('messages.job_application.candidate_name'), 'candidate.user.first_name')
            ->sortable(function (Builder $query, $direction) {
                return $query->orderBy(User::select('first_name')->whereColumn('candidates.user_id', 'users.id'), $direction);
