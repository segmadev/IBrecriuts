...
class JobApplicationController extends AppBaseController
{
    ...
    public function index(int $jobId, Request $request): View
    {
        ...
        if (! in_array($jobId, $companyId) && !Auth::user()->hasRole('Admin')) {
            return view('errors.404');
        }

        if (Auth::user()->hasRole('Admin')) {
            $jobApplication = Job::find($jobId);
            $userId = $jobApplication->company_id;
            // dd($userId);
        }
        ...
        // view('jobs.edit', compact('data', 'job', 'cities', 'states', 'countries'));
        return view('jobs.applications', compact('jobId', 'statusArray', 'job', 'jobStage'));
